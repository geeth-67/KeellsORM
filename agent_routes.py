...
from api_models import QueryRequest, QueryResponse, InsertProposalResponse, InsertProposalRequest, \
...
def query_db(request : QueryRequest) -> QueryResponse:
    ...
    try:
        ...
        results = query_db_with_natural_language(request.query, thread_id=thread_id)
        return QueryResponse(query=request.query, result=results , thread_id=thread_id)
...
@router.post("/approve", response_model=InsertApprovalResponse)
